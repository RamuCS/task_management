...
if (isset($_POST['update_user'])) {
    ...
    $assign = mysqli_real_escape_string($mysqli, $_POST['assigned_role']); // Assigned role ID

    // Ensure the role exists
    $role_check = mysqli_query($mysqli, "SELECT * FROM roles WHERE role_id = '$assign'");
    if (mysqli_num_rows($role_check) == 0) {
        echo "<p style='color: red;'>Invalid role selected.</p>";
    } else {
        // Update the user's name and role in the users table
            ...
            <tr>
                ...
                <td>
                       ...
                       while ($role = mysqli_fetch_assoc($rolesResult)) {
                           $selected = ($user_data['role_name'] == $role['role_name']) ? 'selected' : '';
                           echo "<option value=\"{$role['role_name']}\" $selected>" . htmlspecialchars($role['role_name']) . "</option>";
                       }
                       ?>
                   </select>
